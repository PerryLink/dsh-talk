...
describe('apply — assembly', () => {
  ...
  it('emits talk:speech on the registry change feed when a speech event lands', async () => {
    // Regression: the unit registered without `wire`, so it folded and
    // persisted but the registry never emitted frames — the browser's
    // useProjection stayed undefined forever and playback never fired.
    const harness = await mountHarness()
