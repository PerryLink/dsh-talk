...
- `src/speech.ts` / `src/vocabulary.ts` / `src/projection.ts` — the `dsh-talk/speech` session-event merge, the shared payload types, the adaptive append gate (`appendSpeechEvent`: known-vocabulary hosts append plainly, `ignorable`-envelope hosts append marked, envelope-less hosts get no append — the `tool/call` + `tool/result` events remain the reconstructable trail), and the `talk:speech` projection unit (schema = value ∪ null, so the registry's `ZodType<TalkSpeechProjection | null>` matches exactly).
...
`pnpm run typecheck && pnpm run typecheck:ci && pnpm test && pnpm run build && pnpm run verify:self-contained && pnpm run check:lockfile && pnpm run verify:artifacts && pnpm run check:lockfile && pnpm pack`. The plain `typecheck` resolves the local checkout through tsconfig `paths`; `typecheck:ci` resolves the npm-published `0.1.2-rc.1` faces — keep both green.
