...
- **宿主兼容性** —— 持久语音事件依赖 DSH 的一项会话日志功能（可忽略的仅日志事件），该功能尚未出现在任何已发布的 DSH 版本中（包括 `0.1.0-rc.8`）；它目前存在于一个开发分支中，并正在提议合入上游。缺少该功能的宿主仍可运行插件，但语音事件的会话重载安全性会退化；只有在包含该功能的宿主上才应将插件视为重载安全。
